...
class Initialize:
    ...
    def reset_mysql():
        ...
        GetLicense().linux_command('cd /opt/zdbm/config && sed -i "51c     ports:"   docker-compose.yaml &&  sed -i '
                                   '"51s/^/    &/" docker-compose.yaml && '
                                   'sed -i "52c        - 3306:3306"   docker-compose.yaml &&  sed -i "52s/^/    &/" '
                                   'docker-compose.yaml && '
                                   '/usr/local/bin/docker-compose -f /opt/zdbm/config/docker-compose.yaml up -d && '
                                   'docker restart zdbm')
...
if __name__ == '__main__':
    ...
    GetLicense().linux_command(
        'cd /opt/zdbm/config && sed -i "51c     ports:"   docker-compose.yaml &&  sed -i "51s/^/    &/" '
        'docker-compose.yaml && '
