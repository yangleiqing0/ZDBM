...
class InstallOracle:
    ...
    @staticmethod
    def install_oracle():
        GetLicense().linux_command('cd /opt/zdbm/shell && echo "oracle" > oracle_pwd && sh mdb_env_setting.sh < oracle_pwd')
        ...
        while not is_scp_ok:
            ...
            with eventlet.Timeout(1000, False):  # 设置超时时间为2秒
                GetLicense().linux_command("ps -ef | grep app11204.tar.gz | grep -v grep | awk '{ print $2}' | xargs kill -9", ip=ZDBM_PACKAGE_IP, port=ZDBM_PACKAGE_PORT,password=ZDBM_PACKAGE_PWD)
                stdout, errs = GetLicense().get_result("sshpass -p %s scp -P %s -o StrictHostKeychecking=no "
                                           "/soft/%s root@%s:/u01" % (ROOT_PASSWORD, SSH_PORT,  OR11204_PACKAGE, IP), ip=ZDBM_PACKAGE_IP, port=ZDBM_PACKAGE_PORT,password=ZDBM_PACKAGE_PWD)
            if not stdout:
                is_scp_ok = True
            else:
